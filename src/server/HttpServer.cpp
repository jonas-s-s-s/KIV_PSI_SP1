 ...
 */
void HttpServer::onClientWrite(int clientFd)
{
    int bytesReceived = 0;
    std::string rcv;
    ...
    std::cout << "Received " << bytesReceived << " bytes of data from FD" << clientFd << std::endl;

    //Send HTTP hello world to client
    const std::string& httpHello =
 ...
 */
void HttpServer::onClientDisconnect(int clientFd)
{
 ...
 */
void HttpServer::tcpAccept(int serverFd)
{
 ...
 */
void HttpServer::startServer(const std::string& address, uint16_t port)
{
    ...
    if (bind(serverSocketFd, (struct sockaddr*)&localAddr, sizeof(struct sockaddr_in)) != 0)
    {
        throw std::runtime_error(
            "Failed bind server socket. (FD" + std::to_string(serverSocketFd) + ") (Port: " + std::to_string(port)
            + ")");
    }
    ...
    epoll.addEventHandler(serverSocketFd, EPOLLIN | EPOLLOUT, tcpAccept);
}
